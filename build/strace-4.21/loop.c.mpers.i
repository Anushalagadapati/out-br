...
decode_loop_info(struct tcb *const tcp, const kernel_ulong_t addr)
{
 ...
 if (!((tcp)->qual_flg & 0x002)) {
  do { tprintf("%s%s=", (", "), "lo_device"); print_dev_t((info).lo_device); } while (0);
  tprintf(", lo_inode=%" "l""u", (kernel_ulong_t) info.lo_inode);
  do { tprintf("%s%s=", (", "), "lo_rdevice"); print_dev_t((info).lo_rdevice); } while (0);
 }
 ...
 if (!((tcp)->qual_flg & 0x002) || info.lo_encrypt_type != 0) {
  ...
  printxval(loop_crypt_type_options, info.lo_encrypt_type,
   "LO_CRYPT_???");
  tprintf(", lo_encrypt_key_size=%" "u",
   (uint32_t) info.lo_encrypt_key_size);
 }
 tprints(", lo_flags=");
 printflags(loop_flags_options, info.lo_flags, "LO_FLAGS_???");
 do { tprintf("%s%s=", (", "), "lo_name"); print_quoted_cstring((const char *)(info).lo_name, sizeof((info).lo_name)); } while (0);
 if (!((tcp)->qual_flg & 0x002) || info.lo_encrypt_type != 0) {
  const unsigned int lo_encrypt_key_size =
   ((((unsigned) info.lo_encrypt_key_size) < (32)) ? ((unsigned) info.lo_encrypt_key_size) : (32));
  do { tprintf("%s%s=", (", "), "lo_encrypt_key"); print_quoted_string((const char *)(info).lo_encrypt_key, (lo_encrypt_key_size), (0)); } while (0);
 }
 if (!((tcp)->qual_flg & 0x002))
  tprintf(", lo_init=[%#" "l""x" ", %#" "l""x" "]"
...
decode_loop_info64(struct tcb *const tcp, const kernel_ulong_t addr)
{
 ...
 if (umoven_or_printaddr((tcp), (addr), sizeof(*(&info64)), (void *) (&info64)))
  return;
 if (!((tcp)->qual_flg & 0x002)) {
  do { tprintf("%s%s=", ("{"), "lo_device"); print_dev_t((info64).lo_device); } while (0);
  tprintf(", lo_inode=%" "l" "u", (uint64_t) info64.lo_inode);
  do { tprintf("%s%s=", (", "), "lo_rdevice"); print_dev_t((info64).lo_rdevice); } while (0);
  tprintf(", lo_offset=%#" "l" "x" ", lo_sizelimit=%" "l" "u"
   ", lo_number=%" "u",
   (uint64_t) info64.lo_offset,
   (uint64_t) info64.lo_sizelimit,
   (uint32_t) info64.lo_number);
 } else {
  tprintf("{lo_offset=%#" "l" "x" ", lo_number=%" "u",
   (uint64_t) info64.lo_offset,
   (uint32_t) info64.lo_number);
 }
 ...
 do { tprintf("%s%s=", (", "), "lo_file_name"); print_quoted_cstring((const char *)(info64).lo_file_name, sizeof((info64).lo_file_name)); } while (0);
 if (!((tcp)->qual_flg & 0x002) || info64.lo_encrypt_type != 0) {
  do { tprintf("%s%s=", (", "), "lo_crypt_name"); print_quoted_cstring((const char *)(info64).lo_crypt_name, sizeof((info64).lo_crypt_name)); } while (0);
  const unsigned int lo_encrypt_key_size =
   ((((unsigned) info64.lo_encrypt_key_size) < (32)) ? ((unsigned) info64.lo_encrypt_key_size) : (32));
  do { tprintf("%s%s=", (", "), "lo_encrypt_key"); print_quoted_string((const char *)(info64).lo_encrypt_key, (lo_encrypt_key_size), (0)); } while (0);
 }
 if (!((tcp)->qual_flg & 0x002))
  tprintf(", lo_init=[%#" "l" "x" ", %#" "l" "x" "]}",

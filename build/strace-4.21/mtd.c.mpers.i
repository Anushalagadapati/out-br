...
decode_mtd_oob_buf(struct tcb *const tcp, const kernel_ulong_t addr)
{
 ...
 printaddr(((kernel_ulong_t) (unsigned long) (mbuf.ptr)));
 tprints("}");
}
static void
...
decode_nand_oobinfo(struct tcb *const tcp, const kernel_ulong_t addr)
{
 ...
 tprints(", oobfree={");
 for (i = 0; i < (sizeof(ninfo.oobfree) / sizeof((ninfo.oobfree)[0]) + (sizeof(int[-1 + 2 * !!(!__builtin_types_compatible_p(typeof((ninfo.oobfree)), typeof(&(ninfo.oobfree)[0])))]) * 0)); ++i) {
  if (i)
   tprints("}, ");
  tprints("{");
  for (j = 0; j < (sizeof(ninfo.oobfree[0]) / sizeof((ninfo.oobfree[0])[0]) + (sizeof(int[-1 + 2 * !!(!__builtin_types_compatible_p(typeof((ninfo.oobfree[0])), typeof(&(ninfo.oobfree[0])[0])))]) * 0)); ++j) {
   if (j)
    tprints(", ");
 ...
 tprints("}}, eccpos={");
 for (i = 0; i < (sizeof(ninfo.eccpos) / sizeof((ninfo.eccpos)[0]) + (sizeof(int[-1 + 2 * !!(!__builtin_types_compatible_p(typeof((ninfo.eccpos)), typeof(&(ninfo.eccpos)[0])))]) * 0)); ++i) {
  if (i)
   tprints(", ");
...
decode_nand_ecclayout_user(struct tcb *const tcp, const kernel_ulong_t addr)
{
 ...
 for (i = 0; i < (sizeof(nlay.eccpos) / sizeof((nlay.eccpos)[0]) + (sizeof(int[-1 + 2 * !!(!__builtin_types_compatible_p(typeof((nlay.eccpos)), typeof(&(nlay.eccpos)[0])))]) * 0)); ++i) {
  if (i)
   tprints(", ");
  tprintf("%#x", nlay.eccpos[i]);
 }
 tprintf("}, oobavail=%#x, oobfree={", nlay.oobavail);
 for (i = 0; i < (sizeof(nlay.oobfree) / sizeof((nlay.oobfree)[0]) + (sizeof(int[-1 + 2 * !!(!__builtin_types_compatible_p(typeof((nlay.oobfree)), typeof(&(nlay.oobfree)[0])))]) * 0)); ++i) {
  if (i)
   tprints(", ");

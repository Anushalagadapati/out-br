...
printsigval(const siginfo_t *sip)
{
 ...
 printaddr(((kernel_ulong_t) (unsigned long) (sip->_sifields._rt.si_sigval.sival_ptr)));
 tprints("}");
}
static void
...
print_si_info(const siginfo_t *sip)
{
 ...
 if (((sip)->si_code <= 0)) {
  ...
  case 11: case 7:
   tprints(", si_addr=");
   printaddr(((kernel_ulong_t) (unsigned long) (sip->_sifields._sigfault.si_addr)));
   break;
  case 29:
   switch (sip->si_code) {
  ...
  case 31: {
   const char *scname = syscall_name(shuffle_scno(
    (unsigned) sip->_sifields._sigsys._syscall));
   tprints(", si_call_addr=");
   printaddr(((kernel_ulong_t) (unsigned long) (sip->_sifields._sigsys._call_addr)));
   tprints(", si_syscall=");
   if (scname)
    tprintf("%s%s",
     nr_prefix(sip->_sifields._sigsys._syscall), scname);
   else
    tprintf("%u", (unsigned) sip->_sifields._sigsys._syscall);
   tprints(", si_arch=");

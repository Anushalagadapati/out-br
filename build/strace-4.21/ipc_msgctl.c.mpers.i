...
int sys_msgctl(struct tcb *tcp)
{
 if ((!((tcp)->flags & 0x04))) {
  ...
  do { if ((tcp->u_arg[1]) & 0x100) tprints("IPC_64|"); printxval((msgctl_flags), (tcp->u_arg[1]) & ~0x100, "MSG_???"); } while (0);
  tprints(", ");
 } else {
  const kernel_ulong_t addr = tcp->u_arg[(tcp->s_ent->sys_flags & 00002000) ? 3 : 2];

...
{
 print_quoted_string(ifname, 16 + 1, 0x01);
}
static void
     ...
     const kernel_ulong_t arg, const struct_ifreq *const ifr)
{
 ...
 case 0x8916:
 case 0x8915:
  do { tprintf("%s%s=", (""), "ifr_addr"); print_sockaddr(&(*ifr).ifr_ifru.ifru_addr, sizeof((*ifr).ifr_ifru.ifru_addr)); } while (0);
  break;
 case 0x8918:
 case 0x8917:
  do { tprintf("%s%s=", (""), "ifr_dstaddr"); print_sockaddr(&(*ifr).ifr_ifru.ifru_dstaddr, sizeof((*ifr).ifr_ifru.ifru_dstaddr)); } while (0);
  break;
 case 0x891a:
 case 0x8919:
  do { tprintf("%s%s=", (""), "ifr_broadaddr"); print_sockaddr(&(*ifr).ifr_ifru.ifru_broadaddr, sizeof((*ifr).ifr_ifru.ifru_broadaddr)); } while (0);
  break;
 case 0x891c:
 case 0x891b:
  do { tprintf("%s%s=", (""), "ifr_netmask"); print_sockaddr(&(*ifr).ifr_ifru.ifru_netmask, sizeof((*ifr).ifr_ifru.ifru_netmask)); } while (0);
  break;
 case 0x8924:
 case 0x8927: {
  const unsigned char *bytes =
   (unsigned char *) &ifr->ifr_ifru.ifru_hwaddr.sa_data;
 ...
 case 0x8913:
  tprints("ifr_flags=");
  printflags(iffflags, (unsigned short) ifr->ifr_ifru.ifru_flags, "IFF_???");
  break;
 case 0x891e:
 case 0x891d:
  tprintf("ifr_metric=%d", ifr->ifr_ifru.ifru_ivalue);
  break;
 case 0x8922:
 case 0x8921:
  tprintf("ifr_mtu=%d", ifr->ifr_ifru.ifru_mtu);
  break;
 case 0x8930:
 case 0x8929:
  tprints("ifr_slave=");
  print_ifname(ifr->ifr_ifru.ifru_slave);
  break;
 case 0x8943:
 case 0x8942:
  tprintf("ifr_qlen=%d", ifr->ifr_ifru.ifru_ivalue);
  break;
 case 0x8971:
 case 0x8970:
  tprintf("ifr_map={mem_start=%#" "l""x" ", "
   "mem_end=%#" "l""x" ", base_addr=%#x, "
   "irq=%u, dma=%u, port=%u}",
   (kernel_ulong_t) ifr->ifr_ifru.ifru_map.mem_start,
   (kernel_ulong_t) ifr->ifr_ifru.ifru_map.mem_end,
   (unsigned) ifr->ifr_ifru.ifru_map.base_addr,
   (unsigned) ifr->ifr_ifru.ifru_map.irq,
   (unsigned) ifr->ifr_ifru.ifru_map.dma,
   (unsigned) ifr->ifr_ifru.ifru_map.port);
  break;
 }
}
static unsigned int
     ...
     void *dummy)
{
 ...
 tprints("{ifr_name=");
 print_ifname(ifr->ifr_ifrn.ifrn_name);
 do { tprintf("%s%s=", (", "), "ifr_addr"); print_sockaddr(&(*ifr).ifr_ifru.ifru_addr, sizeof((*ifr).ifr_ifru.ifru_addr)); } while (0);
 tprints("}");
 return 1;
}
...
decode_ifconf(struct tcb *const tcp, const kernel_ulong_t addr)
{
 ...
 if (!ifc || umoven((tcp), (addr), sizeof(*(ifc)), (void *) (ifc)) < 0) {
  if ((!((tcp)->flags & 0x04))) {
   ...
   tprints(", ifc_buf=");
   printaddr(((kernel_ulong_t) (unsigned long) (entering_ifc->ifc_ifcu.ifcu_buf)));
   tprints("}");
  }
  return 0200;
 }
 ...
 if (!entering_ifc->ifc_ifcu.ifcu_buf || ((tcp)->u_error != 0)) {
  ...
  print_array(tcp, ((kernel_ulong_t) (unsigned long) (ifc->ifc_ifcu.ifcu_buf)),
       ...
       &ifr, sizeof(ifr),
       umoven_or_printaddr, print_ifconf_ifreq, ((void *)0));
 }
 tprints("}");
 return 0200;
}
MPERS_PRINTER_DECL(int, sock_ioctl, struct tcb *tcp, const unsigned int code, const kernel_ulong_t arg)
{
 ...
 case 0x8971:
  ...
  if (code == 0x8923) {
   tprints("ifr_newname=");
   print_ifname(ifr.ifr_ifru.ifru_newname);
  } else {
 ...
 case 0x8970:
  if ((!((tcp)->flags & 0x04))) {
   ...
   if (umoven_or_printaddr((tcp), (arg), sizeof(*(&ifr)), (void *) (&ifr)))
    break;
   if (0x8910 == code) {
    tprintf("{ifr_index=%d", ifr.ifr_ifru.ifru_ivalue);
   } else {
    tprints("{ifr_name=");
    print_ifname(ifr.ifr_ifrn.ifrn_name);
   }
   ...
   tprints(", ");
   if (umoven((tcp), (arg), sizeof(*(&ifr)), (void *) (&ifr)) < 0) {
    tprints("???}");
    break;
   }
   if (0x8910 == code) {
